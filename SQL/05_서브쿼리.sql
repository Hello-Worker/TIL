...
-- 부서의  이름(dept.dept_name), 소속직원들의 평균 급여(emp.salary) 출력. 
...
and   (job_id <> 'ST_CLERK'
or    job_id is null);
--job_id가 null이거나 ST_CLERK이 아닌 직원중에서 salary가 ST_CLERK의 평균급여보다 적은 직원
...
-- 직원들의 이름(emp_name), 급여(salary), 업무(job_id) 를 sub query를 이용해 조회.
select  emp_name,
        salary,
        job_id
from    emp        
...
-- 내역테이블에서 데이터테이블의 값이 참조되고 있는지(EXISTS) 없는지(NOT EXISTS)
-- 고객중에 (한번이상) 주문한 고객?
**********************************************************************************************************************/
