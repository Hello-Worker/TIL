...
select  distinct emp_name, job --emp_name,job모두 중복인 것만 제거(한 행이 세트)
from emp;
...
select  emp_id,
        ...
        salary,
        comm_pct,
        salary * comm_pct "커미션" --컬럼 * 컬럼: 행단위로 계산. 
from    emp;
...
select  hire_date-30 "입사 30일 전 날짜",
        hire_date "입사일",
        hire_date+30 "입사 30일 후 날짜"
from    emp;        
...
select emp_id, emp_name, salary --칼럼
from emp --테이블
where emp_id in (100,110,120): --행선택 in () 값과 일치하는 행을 조회

--이름이 홍씨 외자
-- %: 0~N개의 모든 문자, _: 1개의 모든 문자
name like '홍%'
name like '홍_' -- 홍 뒤에 1글자는 반드시 와야 함.
name like '김__' -- 김 뒤에 2글자는 반드시 와야 함.
name like '김_%'
;

select * from emp
where comm_pct is null; -- null값을 조회하기 위해서는 is null을 써야함.
--where comm_pct = null; -- null값을 조회하지 않음. 값이 없는 것 이므로
...
select  emp_id,
        emp_name
from    emp
where   emp_name like 'S%'; -- %: 0개이상의 모든글자
...
select  emp_id,
        emp_name
from    emp
where   emp_name like '%#%%' escape '#';--#을 붙이면 글자를 표현한다는 식
...
--where   salary between 4001 and 7999;
...
select  emp_id,
        ...
        salary
from    emp
where   salary not between 4000 and 8000; --4000과 8000은 포함하지 않는다.  salary < 4000 or salary > 8000
...
--일시: 과거 < 미래  '2000/01/01' < '2000/02/01'
...
select * from emp
order by 1 desc; --select 절의 첫번째 컬럼을 기준으로 desc 정렬
